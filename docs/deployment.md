...
`true` takes the **right-most `X-Forwarded-For` hop**. By default Caddy does **not** trust a client-supplied `X-Forwarded-For`: it overwrites the header with the real connecting peer's IP as the sole entry. This was verified empirically against `caddy:2-alpine` — a client that sends `X-Forwarded-For: 1.2.3.4, 9.9.9.9` has those hops **dropped**, and the app receives only the real client IP. So the right-most hop is always the real client and header spoofing is defeated at the proxy before it reaches the app. This is safe **only because the app's HTTP port is never publicly reachable** — the base compose binds it to loopback (R.1) and the production overlay puts Caddy in front as the sole public entrypoint (Phase 3.3 / Phase 4); if the app port were exposed to the internet, an attacker could connect to it directly, bypass Caddy, and forge the header. (Using `true` rather than a `127.0.0.1` trust list also sidesteps a Docker quirk: Caddy reaches the app over the internal bridge network, so the app sees the request arriving from Caddy's container IP — not `127.0.0.1` — which an IP-list mode would otherwise have to enumerate.)
...
> **Real client IP through Docker's published port.** Caddy sees the real internet client only if Docker preserves the source IP when it forwards the published 80/443 to the Caddy container. For genuine external inbound traffic this is the norm (iptables DNAT preserves the source). A local `curl 127.0.0.1` test is **not** representative — loopback traffic to a published port shows the Docker bridge gateway instead (a userland-proxy artifact), which is why this must be confirmed with a real external request in Phase 8. If per-IP limits ever collapse to a single IP, set `"userland-proxy": false` in `/etc/docker/daemon.json` and restart Docker, or run Caddy directly on the host instead of as a container.
